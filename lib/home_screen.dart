...
import 'product_detail.dart'; // Ensure this imports your ProductDetail screen
import 'drawer_menu.dart'; // Ensure this imports your DrawerMenu
    ...
    showDialog(
      ...
      builder: (context) => AlertDialog(
        ...
        actions: [
          ...
          TextButton(
            onPressed: () {
              Navigator.pushReplacementNamed(context, '/'); // Go to login screen
            },
            child: Text('Logout'),
          ),
        ],
    ...
    return Scaffold(
      ...
      appBar: AppBar(
        ...
        actions: [
          ...
          IconButton(
            ...
            onPressed: () {
              // Implement shopping bag functionality
            },
          ),
          SizedBox(width: 16),
          IconButton(
            icon: Icon(Icons.logout, color: Colors.black), // Logout icon
            onPressed: () => _logout(context), // Show the logout confirmation dialog
          ),
          SizedBox(width: 16),
        ],
      ),
                      ...
                      return GestureDetector(
                        ...
                        child: Card(
                          child: Column(
                            ...
                            children: [
                              Image.network(product.image, height: 100), // Display product image
                              Text(product.name, style: TextStyle(fontSize: 20)),
                              SizedBox(height: 10),
                              Text('₱${product.price.toStringAsFixed(2)}', style: TextStyle(fontSize: 16)), // Price in pesos
                            ],
                          ),
                        ),
                      );
      ...
      bottomNavigationBar: BottomNavigationBar(
        ...
        onTap: (index) {
          switch (index) {
            case 1:
              Navigator.pushNamed(context, '/categories'); // Navigate to Categories Screen
              break;
            case 2:
